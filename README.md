...
4. **`make_easy.py`** *(runs by default; skip with `--no-easy`)* — derives
   a second, easier difficulty from the Styled beatmap: lower Difficulty
   settings, and some of the *repetitive* sections' stream density thinned
   — either merged into short sliders (using the same positions
   apply_style.py already chose) or dropped outright — plus more
   predictable, regular hitsounds there. Non-repetitive sections — a
   bridge, an intro/outro, anything that only happens once — are left as
   they were.
...
This writes `output/<Song Title>/<Song Title> (Base|Variety|Styled|Easy).osu`,
and with `--osz` also bundles those four difficulties plus the MP3 into
`output/<Song Title>/<Song Title>.osz` — drag that straight into osu! to
import all four difficulties at once. Once the `.osz` is built, the
intermediate `.osu` files are deleted (everything they hold is already in
the `.osz`); pass `--keep-osu-files` to keep them around too.

The Easy difficulty (`make_easy.py`, see above) is generated by default —
pass `--no-easy` to skip it and produce only the three main difficulties:

```bash
python3 main.py song.mp3 --no-easy --osz
```
...
- **Objects fully overlapping** — `apply_style.py` never mixes a stack and
  ...
  acceptable" guidance.
- **Easy difficulty settings** — `make_easy.py` clamps AR/OD/HP/CS/
  SliderMultiplier directly to the document's Easy-tier ranges (AR ≤ 5,
  OD/HP 1-3, CS ≤ 4, slider velocity ≤ 1.3), not just a relative shift
  from the Styled difficulty's own (Hard/Insane-range) settings. Lowering
  SliderMultiplier rescales every slider's `length` by the same ratio so
  no slider's actual duration changes.
- **Drain time spread** — `make_easy.py` asserts its output's first/last
  object time exactly matches the Styled difficulty's; thinning only ever
  touches objects strictly between them.
